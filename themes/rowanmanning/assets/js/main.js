...
document.addEventListener('DOMContentLoaded', () => {
	// Set up site search
	const siteSearchBoxes = [...document.querySelectorAll('[data-component=site-search]')].map(
		(element) => SiteSearch.init(element)
	);

	// Set up webmention forms
	[...document.querySelectorAll('[data-component=webmention-form]')].map((element) =>
		WebmentionForm.init(element)
	);

	// Set up feed links
	[...document.querySelectorAll('[data-component=feed-link]')].map((element) =>
		FeedLink.init(element)
	);
