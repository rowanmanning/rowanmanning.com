...
describe('simple website', () => {
	...
	describe('home page', () => {
		...
		describe('head', () => {
			...
			it('has icon metadata', () => {
				...
				assert.ok(
					document
						.querySelector('link[rel=apple-touch-icon][sizes="76x76"]')
						?.getAttribute('href'),
					'apple icon 76'
				);
				assert.ok(
				...
				assert.ok(
					document
						.querySelector('link[rel=apple-touch-icon][sizes="152x152"]')
						?.getAttribute('href'),
					'apple icon 152'
				);
				assert.ok(
					document
						.querySelector('meta[name=msapplication-square70x70logo]')
						?.getAttribute('content'),
					'ms icon 70'
				);
				assert.ok(
					document
						.querySelector('meta[name=msapplication-square150x150logo]')
						?.getAttribute('content'),
					'ms icon 150'
				);
			});
			...
			it('does not have a webmention link element', () => {
				const subject = document
					.querySelector('link[rel=webmention]')
					?.getAttribute('href');
				assert.equal(subject, undefined);
			});
						...
						children: [
							{
								...
								properties: {
									url: ['https://simple.mock.local/section001s/item001/'],
									uid: ['https://simple.mock.local/section001s/item001/'],
									name: ['Mock Title Single Page In Section'],
									summary: ['Mock Description Single Page In Section'],
									published: ['2021-01-02T08:00:00Z'],
									category: ['tag001', 'tag002'],
									author: [
										{
											type: ['h-card'],
											properties: {
							...
							},
							{
								...
								properties: {
									url: ['https://simple.mock.local/section001s/item002/'],
									uid: ['https://simple.mock.local/section001s/item002/'],
									name: ['Untitled Section001'],
									summary: [''],
									published: ['2021-01-02T08:00:00Z'],
									author: [
										{
						...
						children: [
							{
								type: ['h-entry'],
								properties: {
									url: ['https://simple.mock.local/section002s/item001/'],
									uid: ['https://simple.mock.local/section002s/item001/'],
									name: ['Mock Title Single Page In Section'],
									summary: ['Mock Description Single Page In Section'],
									published: ['2021-01-01T08:00:00Z'],
									category: ['tag001', 'tag002'],
									author: [
										{
											type: ['h-card'],
	...
	describe('taxonomy page', () => {
		...
		describe('body', () => {
			...
			it('contains a representation of each term in the taxonomy', () => {
				...
				assert.notEqual(
					subject[0].querySelector('a[href="/tags/tag001/"]'),
					null,
					'link 1 match'
				);
				assert.equal(subject[1].textContent.trim(), 'tag002 (2)', 'name 2 match');
				assert.notEqual(
					subject[1].querySelector('a[href="/tags/tag002/"]'),
					null,
					'link 2 match'
				);
			});
	...
	describe('term page', () => {
		...
		describe('body', () => {
			...
			it('contains the page content', () => {
				const subject = findTestElements(document.body, 'content-body')[0]
					?.textContent.trim()
					.replace(/\s+/g, ' ');
				assert.equal(
					subject,
					'Listing 2 pieces of content with the “Mock Title Term” Tag. Mock Content Term'
				);
			});
	...
	describe('home page RSS', () => {
		...
		it('has a channel with feed information', () => {
			...
			assert.equal(
				document.querySelector('channel > lastBuildDate')?.textContent,
	...
	describe('section page RSS', () => {
		...
		it('has a channel with feed information', () => {
			...
			assert.equal(
				...
				'https://simple.mock.local/section001s/index.xml'
			);
			assert.equal(atomLink?.getAttribute('rel'), 'self');
			assert.equal(atomLink?.getAttribute('type'), 'application/rss+xml');
			assert.equal(
				document.querySelector('channel > description')?.textContent,
				'Mock Description Section'
			);
			assert.equal(document.querySelector('channel > language')?.textContent, 'en');
			assert.equal(
				document.querySelector('channel > copyright')?.textContent,
		...
		describe('when the section has no index page', () => {
			...
			it('has defaulted feed information', () => {
				...
				assert.equal(
					document.querySelector('channel > description')?.textContent,
					'Recent content in All Section002s'
				);
			});
		});
	});
	...
	describe('taxonomy page RSS', () => {
		...
		it('has a channel with feed information', () => {
			...
			assert.equal(
				atomLink?.getAttribute('href'),
				'https://simple.mock.local/tags/index.xml'
			);
			assert.equal(atomLink?.getAttribute('rel'), 'self');
			assert.equal(atomLink?.getAttribute('type'), 'application/rss+xml');
			assert.equal(
				document.querySelector('channel > description')?.textContent,
				'Mock Description Taxonomy'
			);
			assert.equal(document.querySelector('channel > language')?.textContent, 'en');
			assert.equal(
				document.querySelector('channel > copyright')?.textContent,
		...
		describe('when the taxonomy has no index page', () => {
			...
			it('has defaulted feed information', () => {
				...
				assert.equal(
					document.querySelector('channel > description')?.textContent,
					'Recent content in All Categories'
				);
			});
		});
	});
	...
	describe('term page RSS', () => {
		...
		it('has a channel with feed information', () => {
			...
			assert.equal(
				atomLink?.getAttribute('href'),
				'https://simple.mock.local/tags/tag001/index.xml'
			);
			assert.equal(atomLink?.getAttribute('rel'), 'self');
			assert.equal(atomLink?.getAttribute('type'), 'application/rss+xml');
			assert.equal(
				document.querySelector('channel > description')?.textContent,
				'Mock Description Term'
			);
			assert.equal(document.querySelector('channel > language')?.textContent, 'en');
			assert.equal(
				document.querySelector('channel > copyright')?.textContent,
	...
	describe('page furniture', () => {
		...
		describe('header', () => {
			...
			it('contains a link to the home page', () => {
				const subject = findTestElements(header, 'header-site-name')[0]?.getAttribute(
					'href'
				);
				assert.equal(subject, 'https://simple.mock.local/');
			});
